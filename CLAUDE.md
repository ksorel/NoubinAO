...
AOPilot centralise trois choses qui aujourd'hui vivent dans des silos séparés (dossier papier, boîte mail, mémoire de quelqu'un) :
...
- **Hors périmètre V1** : formats bailleurs (Banque mondiale, BAD), plus complexes et destinés à des acteurs déjà mieux outillés. **V2 planifiée** une fois le format national maîtrisé et un premier socle de clients payants acquis — c'est à ce moment qu'AOPilot devient aussi l'outil interne de K-Nowledge pour ses propres réponses aux AO bailleurs, bouclant la logique de dogfooding évoquée dans le plan stratégique.
...
| Normalisation en Markdown | `mammoth` (DOCX→HTML) + `turndown` (HTML→MD) ; `pandoc` en CLI pour les cas complexes | Représentation intermédiaire structurée (titres, tableaux) — voir section dédiée ci-dessous |
...
- **Pipeline** : fichier source → extraction (`mammoth` pour DOCX, `pdf-parse` pour PDF texte) → conversion en Markdown (`turndown` depuis le HTML de `mammoth` ; `pandoc` en CLI si la sortie brute est trop dégradée) → stockage du Markdown normalisé à côté du fichier source.
...
- **Logo** : `logo-icon.svg` (icône seule, pour favicon/app icon) et `logo-full.svg` (icône + wordmark, pour l'en-tête et les documents exportés). Icône = aiguille de boussole (bleu primaire + pointe ambre) dans un carré arrondi, symbolisant le pilotage/la direction. Wordmark "AO" en gras bleu primaire + "Pilot" en régulier slate-900, avec la mention secondaire "K-Nowledge" en petit texte sous le wordmark dans `logo-full.svg` — pour rattacher le produit à la marque mère sans fusionner les deux identités visuelles (AOPilot garde sa propre identité de confiance pour les clients, K-Nowledge reste identifiable pour ses propres relations bailleurs).
...
L'interface d'administration de Supabase (le tableau de bord que les développeurs utilisent pour gérer leurs projets) sert de référence esthétique et fonctionnelle pour AOPilot. Éléments à reprendre concrètement :
...
- Ne pas transformer la normalisation Markdown en projet à part entière : c'est une étape du module 3 (extraction de DAO), pas un nouveau module. Rester sur `mammoth`/`turndown`/`pandoc` et l'OCR en fallback ; ne pas construire de parseur maison tant que les outils existants n'ont pas été testés et jugés insuffisants sur un échantillon réel.
