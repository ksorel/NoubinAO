...
| Extraction PDF/Word | `pdfjs-dist`, `mammoth` (npm, gratuits, open-source) | Suffisant pour extraire le texte brut d'un DAO avant analyse ; `pdfjs-dist` donne aussi accès à la taille de police par fragment de texte, utilisée pour la détection de titres (voir section dédiée) |
| Normalisation en Markdown | `mammoth` (DOCX→HTML) + `turndown` (HTML→MD) pour le Word ; `pdfjs-dist` en accès direct (texte + taille de police par ligne) avec détection de titres par ratio de taille de police pour le PDF | Représentation intermédiaire structurée (titres, tableaux) — voir section dédiée ci-dessous |
...
Tout document entrant (DAO uploadé, pièce administrative de la bibliothèque) doit être converti en Markdown avant d'être transmis à l'API Claude, plutôt que d'envoyer du texte brut non structuré. Ce n'est pas le format du fichier source qui coûte des tokens (`pdfjs-dist`/`mammoth` extraient déjà du texte, pas des binaires) — c'est l'absence de structure qui oblige à renvoyer des documents entiers pour que le modèle retrouve lui-même les sections. Le Markdown règle ce problème et permet un découpage par section fiable.

- **Pipeline** : fichier source → extraction (`mammoth` pour DOCX, `pdfjs-dist` en accès direct pour PDF texte) → conversion en Markdown (`turndown` depuis le HTML de `mammoth` pour le DOCX, avec détection de titres par comparaison à une liste de titres de sections connus ; pour le PDF, détection de titres par **taille de police** — une ligne dont la police dépasse d'au moins 15 % la taille la plus fréquente du document et reste courte (≤120 caractères) est marquée comme titre `##`, une approche indépendante de toute liste figée de titres et donc plus robuste sur un DAO au phrasé imprévisible ; `pandoc` écarté — indisponible sur les fonctions serverless Vercel en production, validé au sous-projet 2 du module 3) → stockage du Markdown normalisé à côté du fichier source.
- **Historique** : la détection de titres du PDF est passée par `pdf-parse` + comparaison de texte à une liste de titres connus avant d'être remplacée (2026-09-03) par `pdfjs-dist` en accès direct + détection par taille de police, `pdf-parse` ne donnant accès qu'au texte brut sans aucune métadonnée de mise en forme — la comparaison de texte se faisait piéger par les mentions en passant d'un titre de section dans le corps du texte (ex. "...voir Section III, Critères d'évaluation..."), fragmentant le document. Le DOCX n'a jamais eu ce problème : `mammoth` restitue les styles de titre Word en vrais `<h1>`/`<h2>`, donc la comparaison de texte y reste appropriée.
- **Fallback OCR obligatoire** : une partie des pièces administratives ivoiriennes (attestations, RCCM) sont des scans image, pas du PDF texte — `pdfjs-dist` n'en extrait rien. Prévoir `tesseract.js` ou l'envoi direct des pages scannées à l'API Claude (lecture d'image native) en repli automatique quand l'extraction texte renvoie un résultat vide ou trop court.
