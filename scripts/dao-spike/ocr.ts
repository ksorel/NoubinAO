...
// `pdf-parse` embarque sa propre copie (plus ancienne) de pdfjs-dist en
// dépendance imbriquée. Les deux copies partagent un état global côté
// pdfjs-dist (fake worker Node) : sans fixer explicitement `workerSrc` sur
// NOTRE copie (résolu en URL file://, requis par le loader ESM de Node sous
// Windows), le fake worker peut se retrouver résolu vers celle de pdf-parse,
// provoquant "API version does not match Worker version".
...
export async function rendreImagePage(buffer: Buffer, numeroPage: number): Promise<Buffer> {
  // Si `pdf-texte.ts` (pdf-parse) a déjà tourné dans ce process, il a laissé
